...
- **2026-09-13 Pages 缓存处理**：首次上线验收时公开页已返回新版 `index.html`，但无版本号的 `css/style.css` / `ui.js` / `dialog.js` 仍命中浏览器旧缓存（实测登录卡圆角仍为 3px）。因此 17 个根目录页面统一给关键资源加版本号；二次视觉验收又发现 `.form-box button` 的高优先级旧规则把登录按钮压回 2px，已补同级覆盖并把 CSS 升至 `style.css?v=20260914-ui2`。所有引用 `ui.js` / `dialog.js` 的页面仍使用 `v=20260913-ui`，测试关另给 `game8.js` 加同版号；确保组员打开公开网址即可拿到同一版 UI。
