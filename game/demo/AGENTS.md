...
**规模（按当前仓库实际文件）**：根目录 HTML **17** 个 + `members/` **6** 个 = 23 个页面；JS **22** 个、约 **9229** 行；CSS 1 个、约 **4259** 行；图片 **44** 个（23 个原素材 + 21 个 WebP 运行版，含 `img/europe-map.svg`）；音频 **5** 首。
...
├── css/
│   └── style.css             # 全局样式（约 3928 行）：基础 UI →「2026 UI 重制」→ 每关桌面 → 三语/夜览 → 电影化 UI → 战场反馈精修
├── js/                       # 22 个文件、7798 行
